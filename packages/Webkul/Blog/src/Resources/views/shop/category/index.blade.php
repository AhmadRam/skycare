...
@push('meta')
    <meta name="title"
        content="{{ $category->meta_title != null && $category->meta_title != "" ? $category->meta_title : ($category->name ?? ($blog_seo_meta_title ?? ($channel->home_seo['meta_title']))) }}" />

    <meta name="description"
        content="{{ $category->meta_description != null && $category->meta_description != "" ? $category->meta_description : ($category->name ?? ($blog_seo_meta_keywords ?? ($channel->home_seo['meta_description']))) }}" />

    <meta name="keywords"
        content="{{ $category->meta_keywords != null && $category->meta_keywords != "" ? $category->meta_keywords : ($category->name ?? ($blog_seo_meta_description ?? ($channel->home_seo['meta_keywords']))) }}" />
@endPush

<x-shop::layouts>
    {{-- Page Title --}}
    <x-slot:title>
        {{ ($category->meta_title != null && $category->meta_title != "") ? $category->meta_title : ($category->name ?? ($blog_seo_meta_title ?? ($channel->home_seo['meta_title']))) }}
    </x-slot>
                            ...
                            <div class="full-content-wrapper">
                                ...
                                <section class="blog-hero-wrapper">
                                    <div class="blog-hero-image">
                                        ...
                                        <img src="{{ '/storage/' . (isset($category->image) && !empty($category->image) && !is_null($category->image) ? $category->image : 'placeholder-banner.jpg') }}"
                                            alt="{{ $category->name }}"
                                                                        ...
                                                                        <div class="post-categories">
                                                                            <p>
                                                                                @foreach ($blog->assign_categorys as $assign_category)
                                                                                    <a href="{{ route('shop.blog.category.index', [$assign_category->slug ?? null]) }}"
                                    ...
                                    <div class=" column-3 blog-sidebar">
                                        <div class="row">
                                            <div class="col-lg-12 mb-4 categories">
                                                <h3>{{ __('blog::app.category.categories') }}</h3>
                                                <ul class="list-group">
                                                    @foreach ($categories as $category)
                                                        <li><a href="{{ route('shop.blog.category.index', [$category->slug ?? null]) }}"
                                                ...
                                                <div class="tags-part">
                                                    <h3>{{ __('blog::app.category.tags') }}</h3>
                                                    <div class="tag-list">
                                                        @foreach ($tags as $tag)
                                                            <a href="{{ route('shop.blog.tag.index', [$tag->slug ?? null]) }}"
                                                                role="button"
