                ...
                <form @submit="handleSubmit($event, storeCustomerBillingAddressToCart)">
                    <div
                        ...
                        <template v-if="! isAddressEmpty(customer.cart.billingAddress)">
                            <x-shop::form.control-group.control type="hidden" name="billing_address_id"
                        ...
                        <div class="relative max-w-[414px] p-0 border border-[#e5e5e5] rounded-xl max-sm:flex-wrap select-none cursor-pointer"
                            ...
                            <span class="icon-edit absolute ltr:right-14 rtl:left-14 top-5 text-2xl cursor-pointer"
                                @click="openUpdateOrCreateCustomerAddressForm(address)"></span>
                    ...
                    <!-- Error Message Block -->
                    <v-error-message class="text-red-500 text-xs italic" name="billing_address_id">
                    </v-error-message>
                    ...
                    <div class="flex gap-x-1.5 items-center mt-5 text-sm text-[#6E6E6E] select-none"
                        ...
                        <v-field type="checkbox" name="use_different_address_for_shipping" v-slot="{ field }"
                            :value="true">
                            <input type="checkbox" name="use_different_address_for_shipping"
                                id="use_different_address_for_shipping" class="sr-only peer" v-bind="field"
                    ...
                    <div class="flex justify-end mt-4"
                        ...
                        <x-shop::button class="primary-button py-3 px-11 rounded-2xl" :title="trans('shop::app.checkout.onepage.addresses.billing.proceed')"
...
<template v-else>
    <x-shop::accordion class="!border-b-0">
        ...
        <x-slot:content class="!p-0 mt-8">
            {!! view_render_event(
                'bagisto.shop.checkout.onepage.addresses.customer.billing.update_or_create.back_button.before',
            ) !!}

            <!-- Back Button -->
            <div>
                <a class="flex justify-end" href="javascript:void(0)" @click="closeUpdateOrCreateCustomerAddressForm()">
            ...
            {!! view_render_event(
                'bagisto.shop.checkout.onepage.addresses.customer.billing.update_or_create.back_button.after',
            ) !!}
            ...
            <x-shop::form v-slot="{ meta, errors, values, handleSubmit }" as="div" id="modalForm">
                <form @submit="handleSubmit($event, updateOrCreateCustomerAddress)">
                    ...
                    <x-shop::form.control-group>
                        <x-shop::form.control-group.control type="hidden" name="type" value="billing" />

                        <x-shop::form.control-group.control type="hidden" name="billing.id" ::value="customer.updateOrCreateBillingAddress.params?.id" />
                    </x-shop::form.control-group>
                    ...
                    {!! view_render_event(
                        'bagisto.shop.checkout.onepage.addresses.customer.billing.update_or_create.company_name.after',
                    ) !!}
                    ...
                    <div class="grid grid-cols-2 gap-x-5">
                        ...
                        {!! view_render_event(
                            'bagisto.shop.checkout.onepage.addresses.customer.billing.update_or_create.first_name.after',
                        ) !!}
                        ...
                        {!! view_render_event(
                            'bagisto.shop.checkout.onepage.addresses.customer.billing.update_or_create.last_name.after',
                        ) !!}
                    </div>
                    ...
                    {!! view_render_event(
                        'bagisto.shop.checkout.onepage.addresses.customer.billing.update_or_create.address1.after',
                    ) !!}
                    ...
                    <div class="grid grid-cols-2 gap-x-5">
                        ...
                        <x-shop::form.control-group class="!mb-4">
                            ...
                            <x-shop::form.control-group.control type="select" name="billing.country"
                                ::value="customer.updateOrCreateBillingAddress.params?.country" rules="{{ core()->isCountryRequired() ? 'required' : '' }}"
                        ...
                        <x-shop::form.control-group>
                            ...
                            <template v-if="haveStates(values.billing?.country)">
                                <x-shop::form.control-group.control type="select" name="billing.state"
                                    ...
                                    <option v-for='(state, index) in states[values.billing?.country]'
                                        :value="state.code">
                                        @{{ state.default_name }}
                                    </option>
                                </x-shop::form.control-group.control>
                    ...
                    <div class="grid grid-cols-2 gap-x-5">
                        ...
                        <x-shop::form.control-group>
                            ...
                            <template v-if="haveCities(values.billing?.state)">
                                <x-shop::form.control-group.control type="select" name="billing.city"
                                    ...
                                    <option v-for='(city, index) in cities[values.billing?.state]'
                                        :value="city.code">
                                        @{{ city.default_name }}
                                    </option>
                                </x-shop::form.control-group.control>
                        ...
                        {!! view_render_event(
                            'bagisto.shop.checkout.onepage.addresses.customer.billing.update_or_create.postcode.after',
                        ) !!}
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-x-5" v-if="values.billing?.country == 'KW'">
                        ...
                        <x-shop::form.control-group>
                            ...
                            <x-shop::form.control-group.control type="text" name="billing.address1[0]"
                                ::value="customer.updateOrCreateBillingAddress.params.address1 && customer.updateOrCreateBillingAddress.params.address1.length > 0 ? customer.updateOrCreateBillingAddress.params?.address1[0] : ''" rules="required|address" :label="trans('shop::app.checkout.onepage.addresses.billing.block-address')" :placeholder="trans('shop::app.checkout.onepage.addresses.billing.block-address')" />
                        ...
                        {!! view_render_event('bagisto.shop.checkout.onepage.addresses.customer.billing.update_or_create.block.after') !!}
                        ...
                        <x-shop::form.control-group>
                            ...
                            <x-shop::form.control-group.control type="text" name="billing.address1[1]"
                                ::value="customer.updateOrCreateBillingAddress.params.address1 && customer.updateOrCreateBillingAddress.params.address1.length > 1 ? customer.updateOrCreateBillingAddress.params?.address1[1] : ''" rules="required|address" :label="trans('shop::app.checkout.onepage.addresses.billing.street-address')" :placeholder="trans('shop::app.checkout.onepage.addresses.billing.street-address')" />

                            <x-shop::form.control-group.error class="mb-2" control-name="billing.address1[1]" />
                        </x-shop::form.control-group>

                        {!! view_render_event('bagisto.shop.checkout.onepage.addresses.customer.billing.update_or_create.street.after') !!}
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-x-5" v-if="values.billing?.country == 'KW'">
                        ...
                        <x-shop::form.control-group>
                            ...
                            <x-shop::form.control-group.control type="text" name="billing.address1[2]"
                                ::value="customer.updateOrCreateBillingAddress.params.address1 && customer.updateOrCreateBillingAddress.params.address1.length > 2 ? customer.updateOrCreateBillingAddress.params?.address1[2] : ''" rules="required|address" :label="trans('shop::app.checkout.onepage.addresses.billing.floor-address')" :placeholder="trans('shop::app.checkout.onepage.addresses.billing.floor-address')" />

                            <x-shop::form.control-group.error class="mb-2" control-name="billing.address1[2]" />
                        </x-shop::form.control-group>

                        {!! view_render_event('bagisto.shop.checkout.onepage.addresses.customer.billing.update_or_create.floor.after') !!}
                        ...
                        <x-shop::form.control-group>
                            ...
                            <x-shop::form.control-group.control type="text" name="billing.address1[3]"
                                ::value="customer.updateOrCreateBillingAddress.params.address1 && customer.updateOrCreateBillingAddress.params.address1.length > 3 ? customer.updateOrCreateBillingAddress.params?.address1[3] : ''" rules="required|address" :label="trans('shop::app.checkout.onepage.addresses.billing.house-address')" :placeholder="trans('shop::app.checkout.onepage.addresses.billing.house-address')" />

                            <x-shop::form.control-group.error class="mb-2" control-name="billing.address1[3]" />
                        </x-shop::form.control-group>

                        {!! view_render_event('bagisto.shop.checkout.onepage.addresses.customer.billing.update_or_create.house.after') !!}
                    </div>
                    ...
                    <!-- Save Button -->
                    <div class="flex justify-end mt-4">
                        <x-shop::button class="primary-button py-3 px-11 rounded-2xl" :title="trans('shop::app.checkout.onepage.addresses.billing.save')"
                            :loading="false" v-if="! isLoading" />
